...
const postAttendanceTokenPage = async (
  ...
  if (
    ...
    return redirection(res, `${BASE_URL}?passwordsNotMatching${QUERY_VALUE}`); // TODO Error Path
  }

  if (await attendanceTokenModel.emailInUse(e, BASE_URL, res)) {
    return redirection(res, `${BASE_URL}?emailInUse${QUERY_VALUE}`); // TODO Error Path
  }

  const tokenModel: AwaitData = await attendanceTokenModel.createAttendanceToken(
    e,
    BASE_URL
    res
  );
  ...
  if (tokenModel) {
    ...
    if (attendanceEmailTokenSent) {
      redirection(res, `${BASE_URL}?emailSent${QUERY_VALUE}`); // TODO Success Path
    }
  }
};
